...
import com.nasr.searchservice.enumeration.SearchQueryType;
...
import static com.nasr.searchservice.enumeration.SearchQueryType.FULL_TEXT_SEARCH;
import static com.nasr.searchservice.enumeration.SearchQueryType.SEMANTIC_SEARCH;
...
public class WebpageServiceImpl implements WebpageService {
    ...
    @Override
    public void saveAll(List<ETLData> etlWebpageData) {
        List<WebpageEntity> webpages = etlWebpageData.stream().map(this::convertDTOToEntity).toList();
        repository.saveAll(webpages);
    }

    @Override
    public List<SearchDto> search(String query, Pageable pageable) throws IOException {
        ...
        SearchResponse<WebpageEntity> response = elasticsearchClient.search(SearchRequest.of(builder -> builder.index(INDEX_NAME)
                ...
                .query(q -> q.match(m -> m.field("body").fuzziness("2").query(query).operator(And)))
                .highlight(b -> b.fields("body", getHighlight()))
        ), WebpageEntity.class);

        return getSearchDtos(response, FULL_TEXT_SEARCH);

    }

    @Override
    public List<SearchDto> embedSearch(String query, Pageable pageable) throws IOException {
        ...
        return getSearchDtos(response, SEMANTIC_SEARCH);
    }

    private List<SearchDto> getSearchDtos(SearchResponse<WebpageEntity> response, SearchQueryType searchQueryType) {
        return response.hits().hits().stream().map(h -> SearchDto.builder()
                .id(h.source().getId())
                .score(h.score())
                .body(h.source().getBody())
                .bodySummary(searchQueryType == FULL_TEXT_SEARCH ? extractBodySummaryFromHighlight(h) : "")
                .build()).toList();
    }
